...
### Changed
- `AgentFactory::addTools()` / `addResources()` / `addSubagents()` now throw `DuplicateToolNameException` on a duplicate tool name instead of registering two definitions of the same name. Duplicates sent the LLM the same name twice while `ToolRegistry` resolved it to whichever was registered last — this surfaces on the same resource added twice, two subagent pools sharing an agent name, and two URIs with the same path (`app://self/article` and `page://self/article`). Rename one of them with `#[Tool(name: ...)]`
